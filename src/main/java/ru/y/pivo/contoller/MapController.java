...
import ru.y.pivo.repos.ProductPerUserRepo;
...
public class MapController {
    ...
    public String nameFilter(@RequestParam String name, Map<String, Object> model) throws Exception {
        ...
        ProductPerUser productPerUser = productPerUserRepo.findByUserAndProduct(user, product);

        if (productPerUser == null) {
            productPerUser = new ProductPerUser(product, user);
            productPerUser.setSearches(1);
            productPerUserRepo.save(
                    productPerUser
            );
        } else {
            productPerUser.setSearches(productPerUser.getSearches()+1);
            productPerUserRepo.save(productPerUser);
        }
    ...
    public String add(@RequestParam String name, @RequestParam String address, Map<String, Object> model) throws Exception {
        ...
        if (product == null) {
            ...
            productRepo.save(product);
            ProductPerUser productPerUser = new ProductPerUser(product, user);
            productPerUserRepo.save(
                    productPerUser);
